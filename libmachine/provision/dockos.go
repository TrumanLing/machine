...
func init() {
	Register("DockOS", &RegisteredProvisioner{
		New: NewDockOSProvisioner,
	})
}

func NewDockOSProvisioner(d drivers.Driver) Provisioner {
	return &DockOSProvisioner{
		Driver: d,
	}
}

type DockOSProvisioner struct {
...
func (provisioner *DockOSProvisioner) Service(name string, action pkgaction.ServiceAction) error {
	var (
		err error
	)
...
func (provisioner *DockOSProvisioner) upgradeIso() error {
	...
	// the latest DockOS ISO.
	if err := b2dutils.DownloadLatestBoot2Docker(); err != nil {
		return err
	}

	// Copy the latest version of DockOS ISO to the machine's directory
...
func (provisioner *DockOSProvisioner) Package(name string, action pkgaction.PackageAction) error {
...
func (provisioner *DockOSProvisioner) Hostname() (string, error) {
	return provisioner.SSHCommand("hostname")
}

func (provisioner *DockOSProvisioner) SetHostname(hostname string) error {
	if _, err := provisioner.SSHCommand(fmt.Sprintf(
		"sudo hostname %s && echo %q | sudo tee /var/lib/DockOS/etc/hostname",
		hostname,
		hostname,
	)); err != nil {
		return err
...
func (provisioner *DockOSProvisioner) GetDockerOptionsDir() string {
	return "/var/lib/DockOS"
}

func (provisioner *DockOSProvisioner) GetAuthOptions() auth.AuthOptions {
	return provisioner.AuthOptions
}

func (provisioner *DockOSProvisioner) GenerateDockerOptions(dockerPort int) (*DockerOptions, error) {
	var (
...
func (provisioner *DockOSProvisioner) CompatibleWithHost() bool {
	return provisioner.OsReleaseInfo.Id == "DockOS"
}

func (provisioner *DockOSProvisioner) SetOsReleaseInfo(info *OsRelease) {
	provisioner.OsReleaseInfo = info
}

func (provisioner *DockOSProvisioner) Provision(swarmOptions swarm.SwarmOptions, authOptions auth.AuthOptions, engineOptions engine.EngineOptions) error {
...
func (provisioner *DockOSProvisioner) SSHCommand(args string) (string, error) {
	return drivers.RunSSHCommandFromDriver(provisioner.Driver, args)
}

func (provisioner *DockOSProvisioner) GetDriver() drivers.Driver {
